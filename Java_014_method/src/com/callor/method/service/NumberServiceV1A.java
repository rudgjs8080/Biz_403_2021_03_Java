 ...
 * 한 개의 클래스(파일)내에서 같은 이름의 함수(method)를
 * 중복하여 선언할 수 있다
 ...
 * method 이름은 addNum()
 * 두 개의 정수를 매개변수로 받는다
 * 		누구나 어디선가 addNum()를 호출 할 때는 Ex) 객체.addNum()
 * 		2개의 정수를 전달하게 하겠다 라는 선언 Ex) 객체.addNum(100, 200)
 * 두 개의 정수를 전달받기 위해서는 method가 준비를 해야한다
 * 		method의 ()안에 변수를 선언한다
 * 		
 * 	두 수의 합을 계산하여 결과가 짝수이면 그 합을 return 하고
 *  그렇지 않으면 null을 return 하라
 *  null 값을 return 해야하기 때문에 Integer를 사용해야 한다
 *  
 *  단순히 정수만 return 하는 경우에는
 *  int를 이용해도 되지만 null 값을 return 해야하기 때문에
 *  Integer 를 사용해야한다
 *  
...
//	}
//	return sum;
	}// addNum(int num1, int num2)가 끝난곳
 ...
 * 
 * method를 호출 할 때 전달하는 데이터의 type에 맞도록
