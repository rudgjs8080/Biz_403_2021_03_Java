...
public class ScoreServiceV1 {
	...
	public Integer inputScore() {
		...
		System.out.println(strNum + " 번의 성적을 입력해주세요");
		...
		int intEng = 0;
		while (true) {
			System.out.println(strNum + " 번의 영어 점수를 입력하세요(중단 : -1)");
			intEng = scan.nextInt();
			if (intEng == -1) {
				return -1;
		...
		int intMath = 0;
		while (true) {
			System.out.println(strNum + " 번의 수학 점수를 입력하세요(중단 : -1)");
			intMath = scan.nextInt();
			if (intMath == -1) {
				return -1;
			} else if (intMath < 0 || intMath > 100) {
				System.out.println("수학점수는 0~100까지");
				continue;
			}
			break;
	...
	public void printScore() {
		...
		for (int i = 0; i < scoreList.size(); i++) {
			...
			System.out.print((sVO.kor + sVO.eng + sVO.math) + "\t");
			System.out.print(((float) (sVO.kor + sVO.eng + sVO.math))/3 + "\n");
